...
public class DatabaseMetaUtil {
    ...
    public static Map<String, Object> databaseInfo(Connection connection) {
        try {
            ...
            return Map.of("databaseProductName",dbmd.getDatabaseProductName(),
                    ...
                    "sqlKeywords",dbmd.getSQLKeywords()
                    );
        } catch (Exception e) {
    ...
    public static List<Map<String,Object>> tableTypeList(DatabaseMetaData databaseMetaData, String catalog, String
        ...
        try (ResultSet rs = databaseMetaData.getTables(catalog,
                (CharSequenceUtil.isNotBlank(schemaPattern) ? schemaPattern.toUpperCase() : schemaPattern), "%",
                types)) {
            ...
            throw new MCPException("failed to list table: "+e.getMessage(),e);
        }
    }
    ...
    public static List<Map<String,Object>> tableColumnsList(Connection connection, String schema, String tableName) {
        return tableColumnsList(getDatabaseMetaData(connection), schema, tableName);
    }
     ...
     * @return 字段信息集合
     */
    public static List<Map<String,Object>> tableColumnsList(DatabaseMetaData databaseMetaData, String schema, String tableName) {
        if (databaseMetaData == null || CharSequenceUtil.isBlank(tableName)) {
            throw new MCPException("table is null");
        }
        try (ResultSet rs = databaseMetaData.getColumns(null,
                CharSequenceUtil.isNotBlank(schema) ? schema.toUpperCase() : schema ,
                CharSequenceUtil.isNotBlank(tableName) ? tableName.toUpperCase() : tableName, null)) {
            ...
            throw new MCPException("failed to describe table: "+e.getMessage(),e);
        }
    }
